...
int updateMethodParamTypes(RequestHdr *hdr) {
  ...
  cls = getConstClass((char*)req->op.nameSpace.data, (char*)req->op.className.data);
  if (!cls) {
  ...
  for (ptok = req->paramValues.first; ptok; ptok = ptok->next) {
    ...
    for (p = 0, pm = ClClassGetMethParameterCount(cl, i); p < pm; p++) {
      ...
      if (strcasecmp(sname, ptok->name) == 0) {
        ...
        param=((ClParameter*)ClObjectGetClSection(&cl->hdr,&meth->parameters))+p;
        break;
      }
    }
    if (p == pm) {
    ...
    /* special case: EmbeddedInstance. Parser will set type to instance, but repository
