...
static int testNameSpace(char *ns, CMPIStatus *st)
{
    ...
    if (interOpProvInfoPtr==forceNoProvInfoPtr) {
       if (strcasecmp(ns,"root/interop")==0 ||
           strcasecmp(ns,"root/pg_interop")==0) {
                         ...
                         const char *role, const char *resultRole)
{
   ...
   if (cc!=NULL && cc->ft->isAssociation(cc) != 0 &&
       (role==NULL || (cc->ft->getProperty(cc,role,NULL,NULL).state & CMPI_notFound)!=0) &&
       (resultRole==NULL || (cc->ft->getProperty(cc,resultRole,NULL,NULL).state & CMPI_notFound)!=0)) {
      _SFCB_RETURN(cc);
   }   
