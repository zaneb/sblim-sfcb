...
$output=`wbemcli -noverify ei $protocol://localhost:$port/root/cimv2:bogusclass 2>&1 1>/dev/null`;
unless ($output =~ /CIM_ERR_INVALID_CLASS: Class not found/ ) {
    print "\tIncorrect error message for: \"Invalid class\".\n"; 
    $rc=1;
}

$output=`wbemcli -noverify ei $protocol://localhost:$port/root/bogus:CIM_LogicalElement 2>&1 1>/dev/null`;
...
$output=`wbemcli -noverify bogus $protocol://localhost:$port/root/cimv2:CIM_LogicalElement 2>&1 1>/dev/null`;
